//Items in Range
// Give a vector of product prices,search for items within a specified price range
...
impl Node {
    ...
    fn insert(&mut self, value: i32) {
        if value > self.val {
            match self.right {
                None => {
                    self.right = Some(Box::new(Node { val: value, left: None, right: None }));
                }
            ...
            match self.left {
                None => {
                    self.left = Some(Box::new(Node { val: value, left: None, right: None }));
                }
                Some(ref mut node) => node.insert(value),
            }
        }
...
fn main() {
    ...
    let result = productsInRange(bst.root.clone(), 7, 20);
    println!("{:?}", result);
}
